let product_fruit = [
    {
        tensp: "Táo xanh Mỹ",
        hinh: "/admin/asset/img/sanpham/taoxanhmy.jpg",
        danhmuc: ["tcvietnam", "muaha"],
        giagoc: "255.000 VNĐ",
        giadagiam: "255.000 VNĐ",
    },
    {
        tensp: "Sầu riêng",
        hinh: "/admin/asset/img/sanpham/raurieng.jpg",
        danhmuc: ["tcvietnam", "muaha"],
        giagoc: "150.000 VNĐ",
        giadagiam: "119.000 VNĐ"
    },
    {
        tensp: "Xoài cát Hòa Lộc",
        hinh: "/admin/asset/img/sanpham/xoaicathoaloc.webp",
        danhmuc: ["tcvietnam", "muaxuan"],
        giagoc: "90.000 VNĐ",
        giadagiam: "75.000 VNĐ",

    },
    {
        tensp: "Chuối Nam Mỹ",
        hinh: "/admin/asset/img/sanpham/chuoinammy.png",
        danhmuc: ["tcnhapkhau", "muaxuan"],
        giagoc: "50.000 VNĐ",
        giadagiam: "39.000 VNĐ"
    },
    {
        tensp: "Cam Úc",
        hinh: "/admin/asset/img/sanpham/camuc.jpg",
        danhmuc: ["tcnhapkhau", "muaxuan"],
        giagoc: "120.000 VNĐ",
        giadagiam: "95.000 VNĐ"
    },
    {
        tensp: "Bưởi da xanh",
        hinh: "/admin/asset/img/sanpham/buoidaxanh.jpg",
        danhmuc: ["tcvietnam", "muaha"],
        giagoc: "110.000 VNĐ",
        giadagiam: "85.000 VNĐ"
    },
    {
        tensp: "Nho đen không hạt",
        hinh: "/admin/asset/img/sanpham/nhodenkhonghat.jpg",
        danhmuc: ["tcnhapkhau", "muaha"],
        giagoc: "190.000 VNĐ",
        giadagiam: "149.000 VNĐ"
    },
    {
        tensp: "Dưa hấu Long An",
        hinh: "/admin/asset/img/sanpham/duahaulongan.png",
        danhmuc: ["tcvietnam", "muaha"],
        giagoc: "35.000 VNĐ",
        giadagiam: "29.000 VNĐ"
    },
    {
        tensp: "Lê Hàn Quốc",
        hinh: "/admin/asset/img/sanpham/lehanquoc.jpg",
        danhmuc: ["tcnhapkhau", "muaha"],
        giagoc: "170.000 VNĐ",
        giadagiam: "135.000 VNĐ"
    },
    {
        tensp: "Ổi ruột đỏ",
        hinh: "/admin/asset/img/sanpham/oiruotdo.png",
        danhmuc: ["tcvietnam", "muathu"],
        giagoc: "40.000 VNĐ",
        giadagiam: "32.000 VNĐ"
    },
    {
        tensp: "Dứa (Thơm) Queen",
        hinh: "/admin/asset/img/sanpham/duaqueen.png",
        danhmuc: ["tcvietnam", "muadong", "hot"],
        giagoc: "45.000 VNĐ",
        giadagiam: "35.000 VNĐ"
    },
    {
        tensp: "Lựu Ấn Độ",
        hinh: "/admin/asset/img/sanpham/luaando.png",
        danhmuc: ["tcnhapkhau", "muaha"],
        giagoc: "180.000 VNĐ",
        giadagiam: "145.000 VNĐ"
    },
    {
        tensp: "Dâu tây Đà Lạt",
        hinh: "/admin/asset/img/sanpham/daudalat.png",
        danhmuc: ["tcvietnam", "muadong"],
        giagoc: "200.000 VNĐ",
        giadagiam: "165.000 VNĐ"
    },
    {
        tensp: "Kiwi vàng New Zealand",
        hinh: "/admin/asset/img/sanpham/kiwinewzealand.png",
        danhmuc: ["tcnhapkhau", "muadong"],
        giagoc: "220.000 VNĐ",
        giadagiam: "185.000 VNĐ"
    },
    {
        tensp: "Mít Thái",
        hinh: "/admin/asset/img/sanpham/mitthai.png",
        danhmuc: ["tcvietnam", "muathu"],
        giagoc: "60.000 VNĐ",
        giadagiam: "49.000 VNĐ"
    },
    {
        tensp: "Thanh long ruột đỏ",
        hinh: "/admin/asset/img/sanpham/thanhlongruotdo.png",
        danhmuc: ["tcvietnam", "muadong"],
        giagoc: "45.000 VNĐ",
        giadagiam: "35.000 VNĐ"
    },
    {
        tensp: "Sapoche",
        hinh: "/admin/asset/img/sanpham/sapoche.png",
        danhmuc: ["tcvietnam", "muadong"],
        giagoc: "35.000 VNĐ",
        giadagiam: "27.000 VNĐ"
    },
    {
        tensp: "Chôm chôm",
        hinh: "/admin/asset/img/sanpham/chomchom.png",
        danhmuc: ["tcvietnam", "muaha"],
        giagoc: "60.000 VNĐ",
        giadagiam: "49.000 VNĐ"
    },
    {
        tensp: "Mận hậu",
        hinh: "/admin/asset/img/sanpham/manhau.jpg",
        danhmuc: ["tcvietnam", "muaha"],
        giagoc: "75.000 VNĐ",
        giadagiam: "60.000 VNĐ"
    },
    {
        tensp: "Me Thái",
        hinh: "/admin/asset/img/sanpham/methai.jpg",
        danhmuc: ["tcnhapkhau", "muadong"],
        giagoc: "90.000 VNĐ",
        giadagiam: "70.000 VNĐ"
    },
    {
        tensp: "Táo đỏ Mỹ",
        hinh: "/admin/asset/img/sanpham/taodomy.jpg",
        danhmuc: ["tcnhapkhau", "muaxuan"],
        giagoc: "240.000 VNĐ",
        giadagiam: "210.000 VNĐ"
    },
    {
        tensp: "Quýt đường",
        hinh: "/admin/asset/img/sanpham/quytduong.png",
        danhmuc: ["tcvietnam", "muaxuan"],
        giagoc: "55.000 VNĐ",
        giadagiam: "45.000 VNĐ"
    },
    {
        tensp: "Bơ sáp",
        hinh: "/admin/asset/img/sanpham/bosap.png",
        danhmuc: ["tcvietnam"],
        giagoc: "60.000 VNĐ",
        giadagiam: "48.000 VNĐ"
    },
    {
        tensp: "Vải thiều",
        hinh: "/admin/asset/img/sanpham/vaithieu.png",
        danhmuc: ["tcvietnam"],
        giagoc: "70.000 VNĐ",
        giadagiam: "55.000 VNĐ"
    },
    {
        tensp: "Đào tiên",
        hinh: "/admin/asset/img/sanpham/daotien.png",
        danhmuc: ["tcnhapkhau"],
        giagoc: "230.000 VNĐ",
        giadagiam: "190.000 VNĐ"
    },
    {
        tensp: "Combo Trái cây Đà Lạt",
        hinh: "/admin/asset/img/sanpham/combodalat.jpg",
        danhmuc: ["tcnhapkhau", "combo", "hot"],
        giagoc: "500.000 VNĐ",
        giadagiam: "399.000 VNĐ"
    },
    {
        tensp: "Giỏ quà Trái cây 130",
        hinh: "/admin/asset/img/sanpham/gioqua130.jpg",
        danhmuc: ["tcvietnam", "combo", "hot"],
        giagoc: "500.000 VNĐ",
        giadagiam: "399.000 VNĐ"
    },
    {
        tensp: "Combo Vitamin C Tươi Mát",
        hinh: "/admin/asset/img/sanpham/combovitaminc.png",
        danhmuc: ["combo", "hot"],
        giagoc: "450.000 VNĐ",
        giadagiam: "349.000 VNĐ"
    },
    {
        tensp: "Combo Trái cây nhiệt đới",
        hinh: "/admin/asset/img/sanpham/combonhietdoi.png",
        danhmuc: ["combo", "hot"],
        giagoc: "520.000 VNĐ",
        giadagiam: "410.000 VNĐ"
    },
    {
        tensp: "Combo Dinh dưỡng mỗi ngày",
        hinh: "/admin/asset/img/sanpham/combodinhduong.png",
        danhmuc: ["combo", "hot"],
        giagoc: "480.000 VNĐ",
        giadagiam: "385.000 VNĐ"
    },
    {
        tensp: "Combo Trái cây giảm cân",
        hinh: "/admin/asset/img/sanpham/combogiamcan.png",
        danhmuc: ["combo", "hot"],
        giagoc: "430.000 VNĐ",
        giadagiam: "329.000 VNĐ"
    },
    {
        tensp: "Combo Sức khỏe gia đình",
        hinh: "/admin/asset/img/sanpham/combosuckhoegiadinh.png",
        danhmuc: ["combo", "hot"],
        giagoc: "550.000 VNĐ",
        giadagiam: "429.000 VNĐ"
    },
    {
        tensp: "Combo Premium nhập khẩu",
        hinh: "/admin/asset/img/sanpham/combonhapkhau.png",
        danhmuc: ["combo", "hot"],
        giagoc: "680.000 VNĐ",
        giadagiam: "559.000 VNĐ"
    },
    {
        tensp: "Giỏ quà Tết Sum Vầy",
        hinh: "/admin/asset/img/sanpham/combotet.png",
        danhmuc: ["gioqua", "hot"],
        giagoc: "650.000 VNĐ",
        giadagiam: "499.000 VNĐ"
    },
    {
        tensp: "Giỏ quà Trái cây Xuân",
        hinh: "/admin/asset/img/sanpham/gioquatraicayxuan.jpg",
        danhmuc: ["gioqua", "hot"],
        giagoc: "600.000 VNĐ",
        giadagiam: "470.000 VNĐ"
    },
    {
        tensp: "Giỏ quà Sức khỏe",
        hinh: "/admin/asset/img/sanpham/gioquasuckhoe.png",
        danhmuc: ["gioqua", "hot"],
        giagoc: "580.000 VNĐ",
        giadagiam: "459.000 VNĐ"
    },
    {
        tensp: "Giỏ quà Doanh nghiệp",
        hinh: "/admin/asset/img/sanpham/gioquadoanhnghiep.webp",
        danhmuc: ["gioqua", "hot"],
        giagoc: "720.000 VNĐ",
        giadagiam: "589.000 VNĐ"
    },
    {
        tensp: "Giỏ quà Trái cây VIP",
        hinh: "/admin/asset/img/sanpham/gioquatraicayvip.jpg",
        danhmuc: ["gioqua", "hot"],
        giagoc: "800.000 VNĐ",
        giadagiam: "639.000 VNĐ"
    },
    {
        tensp: "Giỏ quà Trái cây mix hạt",
        hinh: "/admin/asset/img/sanpham/gioquatcmixhat.jpg",
        danhmuc: ["gioqua", "hot"],
        giagoc: "550.000 VNĐ",
        giadagiam: "439.000 VNĐ"
    },
    {
        tensp: "Gomu Gomu nomi - Model Nika",
        hinh: "/admin/asset/img/sanpham/taqcaosu.jpg",
        danhmuc: ["hot"],
        giagoc: "100000000.000 VNĐ",
        giadagiam: "9990000.000 VNĐ"
    }
    ,
    {
        tensp: "NHO RUBY ROMAN NHẬT BẢN",
        hinh: "/admin/asset/img/sanpham/nhoruby.png",
        danhmuc: ["hot", "tcnhapkhau"],
        giagoc: "11000.000 VNĐ",
        giadagiam: "10900.000 VNĐ"
    }
    ,
    {
        tensp: "Lửa lửa nomi",
        hinh: "/admin/asset/img/sanpham/taqlua.png",
        danhmuc: ["hot", "muaha"],
        giagoc: "100000000.000 VNĐ",
        giadagiam: "9990000.000 VNĐ"
    }
];


// // Đưa lên Local
localStorage.setItem("product_fruit", JSON.stringify(product_fruit));

// for (let i of product_fruit.data) {
//     // Tạo cột để hiển thị mỗi card (responsive 3 sản phẩm / hàng)
//     let col = document.createElement("div");
//     col.classList.add("col-md-4");

//     // Tạo thẻ card
//     let card = document.createElement("div");
//     card.classList.add("card");
//     card.style.width = "100%";

//     // Hình ảnh
//     let image = document.createElement("img");
//     image.setAttribute("src", i.hinh); // Sử dụng đúng tên thuộc tính hình ảnh
//     image.classList.add("card-img-top");
//     image.setAttribute("alt", i.tensp);
//     image.style.height = "100%";
//     card.appendChild(image);

//     // Thân card
//     let cardBody = document.createElement("div");
//     cardBody.classList.add("card-body");

//     // Tên sản phẩm
//     let title = document.createElement("h5");
//     title.classList.add("card-title");
//     title.innerText = i.tensp;

//     // Giá sản phẩm
//     let price = document.createElement("p");
//     price.classList.add("card-text");
//     price.innerHTML = `
//         <del>${i.giagoc}</del><br>
//         <strong class="text-danger">${i.giadagiam}</strong>
//     `;

//     // Nút mua (tùy chỉnh nếu cần)
//     let button = document.createElement("a");
//     button.classList.add("btn", "btn-primary");
//     button.href = "#";
//     button.innerText = "Xem chi tiết";

//     // Gắn vào cardBody
//     cardBody.appendChild(title);
//     cardBody.appendChild(price);
//     cardBody.appendChild(button);

//     // Gắn cardBody vào card
//     card.appendChild(cardBody);

//     // Gắn card vào col
//     col.appendChild(card);

//     // Thêm vào DOM chính
//     document.getElementById("product-content").appendChild(col);
// }